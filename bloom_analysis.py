...
def plot_variable_h(P, default_B, h):
    ...
    plt.title(f'Analys of the Bloom filter with P = {P} and B = {default_B}. \nThe optimal number of hash functions is {np.round(oh, 3)}, which gives a false positive probability of {(false_positive_probability(P, default_B, oh))}')
    plt.plot([oh], [false_positive_probability(P, default_B, oh)], 'ro', label='Optimal number of hash functions')
    plt.text(oh, (false_positive_probability(P, default_B, oh))+0.01, np.round(oh,2), ha="center")
